...
properties {
    $projectName = "Api.Client.Toolkit"
    $unitTestAssembly = "Api.Client.Toolkit.UnitTests.dll"
    $integrationTestAssembly = "Api.Client.Toolkit.IntegrationTests.dll"
	...
	$deploy_dir = "$base_dir\..\..\NuGet-Packages"
}

task default -depends Init, CommonAssemblyInfo, Compile, Test, Nuget, Deploy -Precondition {
...
task Test {
	...
	if($env:manualbuild = $true) {
		exec {
			& $nunitPath\nunit-console.exe $output_dir\$unitTestAssembly $output_dir\$integrationTestAssembly /xml=$results_dir\TestResult.xml
		}
	} else {
		exec {
...
task Nuget {
	...
	exec { invoke-expression "$nuget pack $source_dir\Api.Client.Toolkit.Core\Api.Client.Toolkit.Core.nuspec -BasePath $nuget_base_path  -Version $nugetBuildNumber -OutputDirectory $nuget_dir" }
	exec { invoke-expression "$nuget pack $source_dir\Api.Client.Toolkit.WebApi\Api.Client.Toolkit.WebApi.nuspec -BasePath $nuget_base_path  -Version $nugetBuildNumber -OutputDirectory $nuget_dir" }
}

task Deploy {
	if($env:manualbuild = $true) {
		cp $nuget_dir\*.nupkg $deploy_dir
	}
}
